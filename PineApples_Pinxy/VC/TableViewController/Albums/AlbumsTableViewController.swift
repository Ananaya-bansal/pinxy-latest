...
class AlbumTableViewController: UITableViewController {
    ...
    // Albums containing event details and images
    let Albums: [Album] = [
        ...
        Album(EventDetails: Event(name: "Manali trip", dateTime: Date(), contacts: []), image: ["pic3", "pic3", "pic3", "pic3", "pic3", "pic3"]),
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowImagesSegue" {
            if let destinationVC = segue.destination as? ViewController,
               let selectedImages = sender as? [String] {
